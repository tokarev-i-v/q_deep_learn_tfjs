...
"use strict"; // An agent is in state0 and does action0
...
class Brain {
  constructor(num_states, num_actions, opt = {}) {
    ...
    this.BATCH_SIZE = 64;
    this.optimizer = tf.train.sgd(0.001); // experience replay
  ...
  policy(s) {
    ...
    let ret = {
      action: action_values.max().dataSync()[0],
      value: action_values.argMax(1).dataSync()[0]
    };
  ...
  trainOnReplayBatch(batchSize, gamma, optimizer) {
    ...
    const lossFunction = () => tf.tidy(() => {
      ...
      const qs = this.onlineNetwork.apply(stateTensor, {
        training: true
    ...
    const grads = tf.variableGrads(lossFunction); // Use the gradients to update the online DQN's weights.

    optimizer.applyGradients(grads.grads);
    tf.dispose(grads); // TODO(cais): Return the loss value here?
  }
